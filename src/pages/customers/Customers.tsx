              ...
              <div key={sj.id} className="flex items-center justify-between p-3 rounded-xl border bg-card hover:bg-muted/50">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {format(new Date(sj.created_at), 'dd MMM yyyy', { locale: localeId })}
                  </p>
                </div>
            ...
            {invoices.length === 0 ? (
              ...
              <div key={inv.id} className="flex items-center justify-between p-3 rounded-xl border bg-card hover:bg-muted/50">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {inv.issued_date ? format(new Date(inv.issued_date), 'dd MMM yyyy', { locale: localeId }) : '-'}
                  </p>
                </div>
                <Badge
            ...
            {credits.length === 0 ? (
              ...
              <div key={c.id} className="flex items-center justify-between p-3 rounded-xl border bg-card hover:bg-muted/50">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {format(new Date(c.created_at), 'dd MMM yyyy', { locale: localeId })}
                  </p>
                </div>
  ...
  const { data: customers = [], isLoading } = useQuery<CustomerWithStats[]>({
    ...
    queryFn: async () => {
      const [custRes, salesRes, invoiceRes] = await Promise.all([
        ...
        supabase
          .from('sales')
          .select('id, credit_customer_name, total_amount, is_credit, credit_settled_at, is_cancelled, created_at')
      ...
      return (custRes.data || []).map(c => {
        ...
        const unpaidInvoice = cInvoices
          .filter((i: any) => i.status !== 'paid')
          .reduce((s: number, i: any) => s + (i.total_amount || 0), 0);

        const cCredits = sales.filter(
          (s: any) => s.is_credit && s.credit_customer_name?.toLowerCase().includes(c.name.toLowerCase())
        );
        const unsettledCredit = cCredits
          .filter((s: any) => !s.credit_settled_at)
    ...
    },
    {
      ...
      cell: c => (
        <span className="text-sm font-semibold text-blue-600 dark:text-blue-400">
          {c.totalOmzet > 0 ? formatRpShort(c.totalOmzet) : '—'}
        </span>
      ),
    },
    {
      ...
      <div className="space-y-6">
        <StatsGrid columns={4}>
          ...
          <StatsCard
            ...
            subtitle={`${customers.filter(c => c.totalPiutang > 0).length} pelanggan`}
            subtitleType={totalPiutang > 0 ? 'danger' : 'normal'}
            icon={<CreditCard className="w-5 h-5" />}
            gradient="orange"
          />
          <StatsCard
            title="Dengan Kontak"
            value={customers.filter(c => c.phone || c.email).length}
        ...
        <BeautifulTable
          ...
          emptyState={{
            ...
            description: 'Tambahkan pelanggan B2B pertama Anda.',
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="rounded-2xl">
          ...
          <CustomerForm
            ...
            onSubmit={() => createCustomer.mutate(formData)}
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="rounded-2xl">
          ...
          <CustomerForm
            ...
            setFormData={setFormData}
            onSubmit={() =>
              selectedCustomer && updateCustomer.mutate({ id: selectedCustomer.id, data: formData })
            }
